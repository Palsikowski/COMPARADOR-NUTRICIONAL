// Shader "Mesh drift" (21st.dev Shader Builder), usado no painel da tela de
// entrada. Fica em arquivo próprio porque é código GLSL de terceiro: copiado
// como veio, sem edição, pra dar pra atualizar depois comparando com a origem.
//
// Uniforms empacotados (o shader expõe apelidos u_* legíveis como macros):
//   u_colors[8] (só os 4 primeiros são usados)
//   u_scene     = vec4(largura, altura, segundos * 0.73, 4.0)
//   u_shape     = vec4(1.10, 0.34, 0.50, 0.00)
//   u_surface   = vec4(2.40, 0.96, -0.10, 0.96)
//   u_finish    = vec4(0.00, 0.36, 0.026, 0.07)
//   u_transform = vec4(1453.0, 0.00, 0.00, 0.0)
//   u_space     = vec4(0.00, 0.00, ponteiro x, ponteiro y)
//   u_cursor    = vec4(presença, 2.0, 0.65, 0.46)
...
// Receita "Mesh drift" com as cores e o ajuste pedidos. Os nomes seguem os do
// builder de origem pra dar pra comparar sem tradução no meio.
export const MESH_DRIFT = {
  colors: [
    [0.063, 0.0, 0.169], // #10002B
    [0.498, 0.0, 1.0], // #7F00FF
    [0.2, 0.682, 0.725], // #33AEB9
    [0.035, 0.125, 0.957], // #0920F4
  ],
  timeScale: 0.73,
  shape: [1.1, 0.34, 0.5, 0.0],
  surface: [2.4, 0.96, -0.1, 0.96],
  finish: [0.0, 0.36, 0.026, 0.07],
  transform: [1453.0, 0.0, 0.0, 0.0],
  space: [0.0, 0.0, 0.0, 0.0],
  cursor: [0.0, 2.0, 0.65, 0.46], // presença 0 = cursor desligado
};
